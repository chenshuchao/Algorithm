...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        ...
        int index = size-1;
        return buildTree(postorder, &index, inorder, 0, size-1);
    }
    TreeNode* buildTree(vector<int>& postorder, int *index, vector<int>& inorder, int in_left, int in_right) {
        ...
        (*index) --;
        proot->right = buildTree(postorder, index, inorder, pos+1, in_right);
        proot->left = buildTree(postorder, index, inorder, in_left, pos-1);
        return proot;
    }
};
...
int main() {
    ...
    int arr1[] = {4, 2, 7, 5, 1, 3, 6};
    int arr2[] = {4, 7, 5, 2, 6, 3, 1};
